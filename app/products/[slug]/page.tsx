  ...
  return (
    <div
      className={cx(
        ...
        forceLight ? "border-[#D0DBEA] bg-white shadow-[0_1px_4px_rgba(11,31,58,0.07)]" : "border-white/10 bg-white/5",
        className
      )}
    >
      {children}
...
// Pie colors: multi-color but same "clean + modern" tone in both themes
const PIE_LIGHT = ["#2563EB", "#06B6D4", "#14B8A6", "#22C55E", "#F59E0B", "#8B5CF6"];
const PIE_DARK = ["#22D3EE", "#60A5FA", "#34D399", "#FBBF24", "#A78BFA", "#F472B6"];
...
export default function ProductDetailPage() {
  ...
  useEffect(() => {
    ...
    const onStorage = (e: StorageEvent) => {
      if (e.key === "theme") setTheme(getThemeFromRootOrStorage());
    };
  ...
  const btnSecondary = cx(
    ...
    fl
      ? "border-[#D0DBEA] bg-white text-[#0B1F3A] hover:bg-[#F4F7FC] focus:ring-slate-300/50"
      : "border-white/15 bg-white/5 text-white hover:bg-white/10 focus:ring-white/20"
  );

  // Performance chart colors (non-monochrome in both themes, like screenshot 4)
  const prismaColor = fl ? "#2563EB" : "#22D3EE";
  const benchColor = fl ? "#F59E0B" : "#FBBF24";
      ...
      <div className={cx("border-b backdrop-blur-md", fl ? "border-[#D0DBEA] bg-white/80" : "border-white/10 bg-transparent")}>
        <div className="mx-auto flex max-w-6xl flex-wrap items-center justify-between gap-3 px-4 py-3 sm:px-6 lg:px-8">
          ...
          <div className="flex items-center gap-2">
            ...
            <Link href="/contact" className={cx("inline-flex items-center justify-center", btnSecondary)}>
              Talk to us
            </Link>
          </div>
        </div>
          ...
          >
            ...
            <SoftShell forceLight={fl} className="p-5 sm:p-6">
              <div className="flex flex-col gap-5 sm:flex-row sm:flex-wrap sm:items-start sm:justify-between">
                ...
                <div className={cx("w-full rounded-2xl border p-4 sm:w-auto sm:min-w-[224px]", ic)}>
                  ...
                  <div className="mt-4 flex gap-2">
                    <Link
                      href="/request-access"
                      className="flex-1 rounded-full bg-[#316cb5] px-4 py-2 text-center text-sm font-semibold text-white transition hover:bg-blue-500"
                    >
                      Invest{" "}
                    </Link>
                    <button type="button" className={cx("flex-1", btnSecondary)}>
                      Download
                    </button>
                  </div>
                </div>
              ...
              <div className="mt-5 grid grid-cols-2 gap-3 sm:grid-cols-4">
                {product.kpis.map((k) => (
                  <StatPill key={k.label} label={k.label} value={k.value} sub={k.sub} forceLight={fl} />
                ))}
              </div>
            </SoftShell>
            ...
            >
              <SoftShell forceLight={fl} className="p-5 sm:p-6">
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    <MiniLabel forceLight={fl}>
                      <span className="mr-2 inline-block h-2 w-2 rounded-full" style={{ background: prismaColor }} />
                      Prisma
                    </MiniLabel>
                    <MiniLabel forceLight={fl}>
                      <span className="mr-2 inline-block h-2 w-2 rounded-full" style={{ background: benchColor }} />
                      Benchmark
                    </MiniLabel>
                  </div>
                </div>
                    ...
                    <AreaChart data={product.series} margin={{ left: 0, right: 10, top: 8, bottom: 0 }}>
                      <defs>
                        <linearGradient id="prismaFill" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="0%" stopColor={prismaColor} stopOpacity={0.18} />
                          <stop offset="100%" stopColor={prismaColor} stopOpacity={0.01} />
                        </linearGradient>
                        <linearGradient id="benchFill" x1="0" y1="0" x2="0" y2="1">
                          <stop offset="0%" stopColor={benchColor} stopOpacity={0.14} />
                          <stop offset="100%" stopColor={benchColor} stopOpacity={0.01} />
                        </linearGradient>
                      </defs>
                      <CartesianGrid strokeDasharray="4 6" stroke={fl ? "#E2EBF4" : "rgba(255,255,255,0.07)"} />
                      <XAxis
                        dataKey="month"
                        tick={{ fontSize: 11, fill: fl ? "#6B7F96" : "rgba(255,255,255,0.4)" }}
                        stroke="transparent"
                      />
                      <YAxis
                        tick={{ fontSize: 11, fill: fl ? "#6B7F96" : "rgba(255,255,255,0.4)" }}
          ...
          >
                      ...
                      <ResponsiveContainer width="100%" height="100%">
                        <PieChart>
                          <Pie data={data} dataKey="value" nameKey="name" innerRadius={46} outerRadius={68} paddingAngle={2}>
                            {data.map((_, i) => (
                              <Cell key={i} fill={pies[(i + offset) % pies.length]} />
                            ))}
                          </Pie>
            ...
            <SoftShell forceLight={fl} className="p-5 sm:p-6">
              <div className="flex items-center justify-between">
                ...
                <Link href="#holdings" className={cx("text-xs font-semibold", fl ? "text-[#2563EB]" : "text-cyan-200")}>
                  View all →
                </Link>
              </div>
                ...
                {product.holdings.slice(0, 4).map((h) => (
                  <div key={h.symbol} className={cx("flex items-center gap-3 rounded-xl border p-3 text-sm", ic)}>
                    ...
                    <div className="min-w-0">
                      ...
                      <p className={cx("text-xs", tm)}>
                        Weight {h.weight.toFixed(1)}% · 1Y {formatPct(h.oneY)}
                      </p>
                    </div>
              ...
              <div className="mt-5 grid gap-3 sm:grid-cols-3">
                {product.buildSteps.map((s, idx) => (
                  <motion.div
                    ...
                    className={cx("rounded-2xl border p-4", ic)}
                  >
                    <div className="flex items-center gap-2">
              ...
              <div className="mt-5 space-y-3">
                ...
                <Link
                  ...
                  className={cx(
                    ...
                    fl ? "border-[#D0DBEA] bg-white text-[#0B1F3A] hover:bg-[#F4F7FC]" : "border-white/15 bg-white/5 text-white hover:bg-white/10"
                  )}
                >
                  Ask a question
                </Link>
                <div className={cx("rounded-2xl border p-4 text-xs", ic)}>
                  ...
                  <p className={cx("mt-1 leading-5", ts)}>
                    Past returns are not indicative of future performance. Review risk and allocation before investing.
                  </p>
                </div>
              </div>
            </SoftShell>
          ...
          <div className={cx("relative overflow-hidden rounded-3xl border p-8 sm:p-10", fl ? "border-[#D0DBEA] bg-white shadow-[0_2px_8px_rgba(11,31,58,0.07)]" : "border-white/10 bg-white/5")}>
            ...
            <div className="relative flex flex-col items-start justify-between gap-6 md:flex-row md:items-center">
              ...
              <div className="flex flex-wrap gap-3">
                ...
                <Link href="/products" className={cx("inline-flex items-center justify-center", btnSecondary)}>
                  View all Products
                </Link>
              </div>
            </div>
